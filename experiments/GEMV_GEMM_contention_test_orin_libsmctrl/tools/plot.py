#!/usr/bin/env python3
"""
Plot GEMV-GEMM contention benchmark results
...
def plot_contention_results(csv_path, output_prefix=None):
    """
    Generate four plots analyzing GEMV-GEMM contention
    ...
    # Validate columns
    required_cols = ['num_sms', 'mode', 'gemv_gflops', 'gemm_gflops', 
                     'gemv_time_ms', 'gemm_time_ms', 'gemv_slowdown', 
    ...
    # Separate by mode
    isolated_gemv = df[df['mode'] == 'isolated_gemv'].sort_values('num_sms')
    ...
    merged = concurrent[['num_sms', 'gemv_gflops', 'gemm_gflops', 'gemv_slowdown', 
                         'gemm_slowdown', 'overlap_pct']].copy()
    merged.rename(columns={
        'gemv_gflops': 'gemv_gflops_concurrent',
    ...
    merged = pd.merge(
        merged,
        isolated_gemv[['num_sms', 'gemv_gflops']].rename(columns={'gemv_gflops': 'gemv_gflops_isolated'}),
        on='num_sms'
    )
    merged = pd.merge(
        merged,
        isolated_gemm[['num_sms', 'gemm_gflops']].rename(columns={'gemm_gflops': 'gemm_gflops_isolated'}),
        on='num_sms'
    )
    
    # Calculate retention percentages
    merged['gemv_retention_pct'] = (merged['gemv_gflops_concurrent'] / 
                                     merged['gemv_gflops_isolated']) * 100
    merged['gemm_retention_pct'] = (merged['gemm_gflops_concurrent'] / 
                                     merged['gemm_gflops_isolated']) * 100
    ...
    ax1.plot(merged['num_sms'], merged['gemv_retention_pct'], 
             marker='o', markersize=8, linewidth=2.5, 
             label='GEMV Throughput Retention', color='#2E86AB')
    ...
    # Left Y-axis: GEMV Throughput (GFLOPS)
    color_gemv = '#2E86AB'
    ax2.set_xlabel('Number of SMs', fontsize=14, fontweight='bold')
    ax2.set_ylabel('GEMV Throughput (GFLOPS)', fontsize=14, fontweight='bold', color=color_gemv)
    
    line1 = ax2.plot(isolated_gemv['num_sms'], isolated_gemv['gemv_gflops'], 
                     marker='o', markersize=8, linewidth=2.5, linestyle='--',
                     label='Isolated GEMV', color=color_gemv, alpha=0.7)
    
    line2 = ax2.plot(concurrent['num_sms'], concurrent['gemv_gflops'], 
                     marker='o', markersize=8, linewidth=2.5,
                     label='Concurrent GEMV', color=color_gemv)
    
    ax2.tick_params(axis='y', labelcolor=color_gemv)
    ax2.fill_between(concurrent['num_sms'], 
                      concurrent['gemv_gflops'], 
                      isolated_gemv['gemv_gflops'],
                      alpha=0.2, color=color_gemv, label='GEMV Degradation')
    
    # Right Y-axis: GEMM Throughput (GFLOPS)
    ax2_right = ax2.twinx()
    color_gemm = '#A23B72'
    ...
    ax3.plot(concurrent['num_sms'], concurrent['gemv_slowdown'], 
             marker='o', markersize=8, linewidth=2.5, 
             label='GEMV Slowdown', color='#2E86AB')
    ...
    ax3.set_ylim(0.9, max(concurrent['gemv_slowdown'].max(), 
    ...
    print(f"\nGEMV Throughput Retention:")
    print(f"  Min:  {merged['gemv_retention_pct'].min():.1f}%")
    print(f"  Max:  {merged['gemv_retention_pct'].max():.1f}%")
    print(f"  Mean: {merged['gemv_retention_pct'].mean():.1f}%")
    ...
    print(f"\nGEMV Slowdown:")
    print(f"  Min:  {concurrent['gemv_slowdown'].min():.2f}x")
    print(f"  Max:  {concurrent['gemv_slowdown'].max():.2f}x")
    print(f"  Mean: {concurrent['gemv_slowdown'].mean():.2f}x")
