...
# JAX profiler helper (works inside JIT-compiled code)
def trace_scope(name: str):
    """Helper to create JAX profiler trace annotation."""
...
class Attention(nn.Module):
    ...
    def __call__(self, xs, positions, attn_mask, kv_cache):
        with trace_scope("attn_qkv_proj"):
        ...
        with trace_scope("attn_kv_cache"):
            if kv_cache is not None:
                cache_k, cache_v = kv_cache
                k = jnp.concatenate([cache_k, k], axis=1)
                v = jnp.concatenate([cache_v, v], axis=1)

        with trace_scope("attn_compute_logits"):
        ...
        with trace_scope("attn_weighted_sum"):
            encoded = jnp.einsum("BKGTS,BSKH->BTKGH", probs, v)
            encoded = einops.rearrange(encoded, "B T K G H -> B T (K G) H")

        with trace_scope("attn_output_proj"):
            out = []
            start = 0
            for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
                if x is not None:
...
class Block(nn.Module):
    ...
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, deterministic=True):  # noqa: FBT002
        with trace_scope("block_pre_attention"):
        ...
        with trace_scope("block_attention"):
            post_attn, kv_cache = attn(pre_attn, positions, attn_mask, kv_cache)
        
        with trace_scope("block_post_attention"):
            post_attn = jax.tree.map(lambda x: drop(x, deterministic), post_attn)
            post_attn = sharding.activation_sharding_constraint(post_attn)
            xs = [_gated_residual(x, y, gate) for x, y, gate in zip(xs, post_attn, gates, strict=True)]
            xs = sharding.activation_sharding_constraint(xs)

        with trace_scope("block_pre_ffn"):
            out = []
            gates = []
            for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
                if x is not None:
        ...
        with trace_scope("block_post_ffn"):
...
class Module(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        with trace_scope("llm_embed_prepare"):
            embedded = jax.tree.map(lambda e: e.astype(self.embed_dtype), embedded)
            mask = jnp.asarray(mask)[:, None, :, :]
            if adarms_cond is None:
                adarms_cond = [None] * len(self.configs)

        with trace_scope("llm_transformer_layers"):
            embedded, kv_cache = self.layers(embedded, kv_cache, positions, mask, adarms_cond, deterministic)

        with trace_scope("llm_final_norm"):
